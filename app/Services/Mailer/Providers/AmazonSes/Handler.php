...
class Handler extends BaseHandler
{
    ...
    public function addNewSenderEmail($connection, $email)
    {
        ...
        if ($emailDomain != $validSenders['verified_domain']) {
            return new \WP_Error(422, __('Invalid email address! Please use a verified domain.', 'websmtp'));
        }
        ...
        if (isset($mappings[$email])) {
            return new \WP_Error(422, __('Email address already exists with another connection. Please choose a different email.', 'websmtp'));
        }
    ...
    public function removeSenderEmail($connection, $email)
    {
        ...
        if (in_array($email, $validSenders['emails'])) {
            return new \WP_Error(422, __('Sorry! you can not remove this email from this connection', 'websmtp'));
        }
        ...
        if (!isset($mappings[$email])) {
            return new \WP_Error(422, __('Email does not exists. Please try again.', 'websmtp'));
        }
